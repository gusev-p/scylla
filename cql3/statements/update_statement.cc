...
update_statement_impl::update_statement_impl(const statement_type type, schema_ptr schema, uint32_t bound_terms, std::unique_ptr<attributes> attrs)
    : modification_statement_impl(type,
        {
            .allow_clustering_key_slices = false,
            .require_full_clustering_key = true,
        }, std::move(schema), bound_terms, std::move(attrs))
{
}
...
insert_prepared_json_statement_impl::insert_prepared_json_statement_impl(schema_ptr schema, uint32_t bound_terms, std::unique_ptr<attributes> attrs, expr::expression v, bool default_unset)
    : update_statement_impl(statement_type::INSERT, schema, bound_terms, std::move(attrs))
    , _value(std::move(v))
    , _default_unset(default_unset)
{
    _restrictions = cql3::restrictions::make_trivial_statement_restrictions(schema, false);
}

json_cache_opt insert_prepared_json_statement_impl::maybe_prepare_json_cache(const query_options& options) const {
    cql3::raw_value c = expr::evaluate(_value, options);
    sstring json_string = utf8_type->to_string(to_bytes(c.view()));
    return json_helpers::parse(std::move(json_string), _schema->all_columns());
}

void
insert_prepared_json_statement_impl::execute_set_value(mutation& m, const clustering_key_prefix& prefix,
...
insert_prepared_json_statement_impl::build_partition_keys(const query_options& options, const json_cache_opt& json_cache) const {
    dht::partition_range_vector ranges;
    std::vector<bytes_opt> exploded;
    for (const auto& def : _schema->partition_key_columns()) {
    ...
    auto pkey = partition_key::from_optional_exploded(*_schema, std::move(exploded));
    validation::validate_cql_key(*_schema, pkey);
    auto k = query::range<query::ring_position>::make_singular(dht::decorate_key(*_schema, std::move(pkey)));
    ranges.emplace_back(std::move(k));
    return ranges;
}

query::clustering_row_ranges insert_prepared_json_statement_impl::create_clustering_ranges(const query_options& options, const json_cache_opt& json_cache) const {
    query::clustering_row_ranges ranges;
    std::vector<bytes_opt> exploded;
    for (const auto& def : _schema->clustering_key_columns()) {
        auto json_value = json_cache->find(def.name_as_text());
        if (json_value == json_cache->end() || !json_value->second) {
            throw exceptions::invalid_request_exception(format("Missing mandatory PRIMARY KEY part {}", def.name_as_text()));
        }
    ...
    auto k = query::range<clustering_key_prefix>::make_singular(clustering_key_prefix::from_optional_exploded(*_schema, std::move(exploded)));
    ranges.emplace_back(query::clustering_range(std::move(k)));
    return ranges;
}

void insert_prepared_json_statement_impl::execute_operations_for_key(mutation& m, const clustering_key_prefix& prefix, const update_parameters& params, const json_cache_opt& json_cache) const {
    for (const auto& def : _schema->regular_columns()) {
...
insert_statement::prepare_internal(data_dictionary::database db, schema_ptr schema, prepare_context& ctx, std::unique_ptr<attributes> attrs) const {
    auto stmt = ::make_shared<cql3::statements::update_statement_impl>(
        cql3::statements::statement_type::INSERT,
        schema,
...
::shared_ptr<const cql3::statements::modification_statement_impl>
insert_json_statement::prepare_internal(data_dictionary::database db, schema_ptr schema, prepare_context& ctx, std::unique_ptr<attributes> attrs) const {
    ...
    auto stmt = ::make_shared<cql3::statements::insert_prepared_json_statement_impl>(schema, 
        ctx.bound_variables_size(),
        std::move(attrs),
        std::move(prepared_json_value),
        _default_unset);
    prepare_conditions(db, *schema, ctx, *stmt);
    return stmt;
}
...
::shared_ptr<const cql3::statements::modification_statement_impl>
update_statement::prepare_internal(data_dictionary::database db, schema_ptr schema, prepare_context& ctx, std::unique_ptr<attributes> attrs) const {
    auto stmt = ::make_shared<cql3::statements::update_statement_impl>(
