...
public:
    ...
    ::shared_ptr<const cql3::statements::modification_statement_impl> prepare_impl(data_dictionary::database db, prepare_context& ctx) const;
    void add_raw(sstring&& raw) { _raw_cql = std::move(raw); }
    const sstring& get_raw_cql() const { return _raw_cql; }
protected:
    virtual ::shared_ptr<const cql3::statements::modification_statement_impl> prepare_internal(
