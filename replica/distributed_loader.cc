...
public:
    ...
    future<> start() {
        ...
        if (!is_system_keyspace(_ks)) {
            co_await smp::invoke_on_all([this] {
                _db.local().mark_table_ready_for_writes(_global_table->schema());
            });
        }
    }
...
future<> distributed_loader::init_system_keyspace(sharded<db::system_keyspace>& sys_ks, distributed<locator::effective_replication_map_factory>& erm_factory, distributed<replica::database>& db, db::config& cfg) {
    population_started = true;

    return seastar::async([&sys_ks, &erm_factory, &db, &cfg] {
        sys_ks.invoke_on_all([&erm_factory, &db, &cfg] (auto& sys_ks) {
            return sys_ks.make(erm_factory.local(), db.local(), cfg);
        }).get();
