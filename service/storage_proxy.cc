...
void remove_non_local_host_ids(auto&& host_ids, const locator::effective_replication_map& erm) {
...
class storage_proxy::remote {
    ...
    paxos::paxos_store _paxos_store;
...
private:
    ...
    future<rpc::no_wait_type> handle_paxos_learn(
        ...
        return handle_write(src_addr, t, schema_version, std::move(decision), std::move(forward), reply_to, std::move(forward_id),reply_to_id, shard,
               ...
               /* apply_fn */ [this] (shared_ptr<storage_proxy>& p, tracing::trace_state_ptr tr_state, schema_ptr s,
                     ...
                     return paxos::paxos_state::learn(*p, _paxos_store, std::move(s), decision, timeout, tr_state);
              },
    ...
    handle_paxos_prepare(
        ...
        co_return co_await _sp.container().invoke_on(shard, _sp._write_smp_service_group, [gs = global_schema_ptr(schema), gt = tracing::global_trace_state_ptr(std::move(tr_state)),
            ...
            auto r = co_await paxos::paxos_state::prepare(sp, paxos_store, tr_state, gs, *cmd, key, ballot, only_digest, da, *timeout);
    ...
    future<bool> handle_paxos_accept(
        ...
        co_return co_await _sp.container().invoke_on(shard, _sp._write_smp_service_group, coroutine::lambda([gs = global_schema_ptr(schema), gt = tracing::global_trace_state_ptr(tr_state),
            ...
            return paxos::paxos_state::accept(sp, _paxos_store, gt, gs, token, proposal, *timeout);
        }));
    }
    ...
    future<rpc::no_wait_type> handle_paxos_prune(
        ...
        co_await smp::submit_to(shard, _sp._write_smp_service_group, [gs = global_schema_ptr(schema), gt = tracing::global_trace_state_ptr(std::move(tr_state)),
                ...
                tracing::trace_state_ptr tr_state = gt;
                co_await paxos::paxos_state::prune(paxos_store, gs, key, ballot,  *timeout, tr_state);
...
future<result<>> storage_proxy::mutate_result(std::vector<mutation> mutations, db::consistency_level cl, clock_type::time_point timeout, tracing::trace_state_ptr tr_state, service_permit permit, db::allow_per_partition_rate_limit allow_limit, bool raw_counters, coordinator_mutate_options options) {
    if (_cdc && _cdc->needs_cdc_augmentation(mutations)) {
        return _cdc->augment_mutation_call(timeout, std::move(mutations), tr_state, cl).then([this, cl, timeout, tr_state, permit = std::move(permit), raw_counters, cdc = _cdc->shared_from_this(), allow_limit, options = std::move(options)](std::tuple<std::vector<mutation>, lw_shared_ptr<cdc::operation_result_tracker>>&& t) mutable {
...
future<result<>> storage_proxy::do_mutate(std::vector<mutation> mutations, db::consistency_level cl, clock_type::time_point timeout, tracing::trace_state_ptr tr_state, service_permit permit, bool raw_counters, db::allow_per_partition_rate_limit allow_limit, lw_shared_ptr<cdc::operation_result_tracker> cdc_tracker, coordinator_mutate_options options) {
 ...
 */
future<>
storage_proxy::mutate_atomically(std::vector<mutation> mutations, db::consistency_level cl, clock_type::time_point timeout, tracing::trace_state_ptr tr_state, service_permit permit, coordinator_mutate_options options) {
    return mutate_atomically_result(std::move(mutations), cl, timeout, std::move(tr_state), std::move(permit), std::move(options))
            .then(utils::result_into_future<result<>>);
}
...
future<result<>>
storage_proxy::mutate_atomically_result(std::vector<mutation> mutations, db::consistency_level cl, clock_type::time_point timeout, tracing::trace_state_ptr tr_state, service_permit permit, coordinator_mutate_options options) {
    utils::latency_counter lc;
    lc.start();
    ...
    public:
        context(storage_proxy & p, std::vector<mutation>&& mutations, lw_shared_ptr<cdc::operation_result_tracker>&& cdc_tracker, db::consistency_level cl, clock_type::time_point timeout, tracing::trace_state_ptr tr_state, service_permit permit, coordinator_mutate_options options)
                : _p(p)
    ...
    auto mk_ctxt = [this, tr_state, timeout, permit = std::move(permit), cl, options = std::move(options)] (std::vector<mutation> mutations, lw_shared_ptr<cdc::operation_result_tracker> tracker) mutable {
      try {
...
result<::shared_ptr<abstract_read_executor>> storage_proxy::get_read_executor(lw_shared_ptr<query::read_command> cmd,
    ...
    host_id_vector_replica_set all_replicas = get_endpoints_for_reading(schema->ks_name(), *erm, token, replica_local);
...
storage_proxy::query_partition_key_range_concurrent(storage_proxy::clock_type::time_point timeout,
    ...
    for (;;) {
        ...
        while (i != ranges.end()) {
            ...
            host_id_vector_replica_set live_endpoints = get_endpoints_for_reading(schema->ks_name(), *erm, end_token(range), replica_local);
            ...
            if (!erm->get_replication_strategy().uses_tablets()) {
                while (i != ranges.end())
                {
                    ...
                    host_id_vector_replica_set next_endpoints = get_endpoints_for_reading(schema->ks_name(), *erm, end_token(next_range), replica_local);
...
host_id_vector_replica_set storage_proxy::get_endpoints_for_reading(const sstring& ks_name, const locator::effective_replication_map& erm, const dht::token& token, locator::replica_local replica_local) const {
