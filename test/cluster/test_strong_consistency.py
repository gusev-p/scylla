...
async def test_task_profiler(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        # Stop profiler and dump. Each shard writes two files per server:
        #   <dump_path>.raw.<shard>   per-leaf sample counts
        #   <dump_path>.norm.<shard>  per-tick presence counts
        # Both files use the folded-stack format and are independently
        ...
        for s in servers:
            ...
            shard_raw = glob.glob(f"{dump_path}.raw.*")
            shard_norm = glob.glob(f"{dump_path}.norm.*")
            assert len(shard_raw) > 0, f"No raw dump files for server {s.server_id}"
            assert len(shard_norm) > 0, f"No normalized dump files for server {s.server_id}"
            assert len(shard_raw) == len(shard_norm), \
                f"raw/normalized file count mismatch for server {s.server_id}"
            raw_files.extend(shard_raw)
            norm_files.extend(shard_norm)

        # Verify folded-stack format and check we got multi-frame stacks
        # in both views.
        def parse_folded(paths):
            total_samples = 0
            max_depth = 0
            for path in paths:
                with open(path) as f:
                    for line in f:
                        ...
                        total_samples += count
            return total_samples, max_depth

        raw_samples, raw_depth = parse_folded(raw_files)
        norm_samples, norm_depth = parse_folded(norm_files)
        logger.info(f"Async profiler raw: {raw_samples} samples, max stack depth {raw_depth}, "
                     f"{len(raw_files)} shard files")
        logger.info(f"Async profiler norm: {norm_samples} samples, max stack depth {norm_depth}, "
                     f"{len(norm_files)} shard files")
        assert raw_samples > 0, "Async profiler raw view produced no samples"
        assert norm_samples > 0, "Async profiler normalized view produced no samples"
