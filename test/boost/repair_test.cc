...
SEASTAR_TEST_CASE(test_reader_with_different_strategies) {
    ...
    // compare its output to multishard_filter.
    ...
    return do_with_cql_env([] (cql_test_env& e) -> future<> {
        ...
        co_await e.db().invoke_on_all([gs = global_schema_ptr(gen.schema())](replica::database& db) {
            auto& ks = db.find_keyspace("ks");

            auto cf_config = ks.make_column_family_config(*gs.get(), db);
            // we don't need disk in this test,
            // and we get some strange errors otherwise
            cf_config.enable_disk_reads = false;
            cf_config.enable_disk_writes = false;
            cf_config.enable_commitlog = false;
            cf_config.enable_cache = false;

            db.add_column_family(ks, gs.get(), cf_config);
            ...
            repair_reader::read_strategy strategy1, repair_reader::read_strategy strategy2) -> future<>
        {
            ...
            const auto remote_shard = tests::random::get_int(remote_sharder.shard_count() - 1);
            ...
            auto create_reader = [&](repair_reader::read_strategy strategy) {
                return repair_reader(e.db(), cf, cf.schema(), make_reader_permit(e),
                    random_range, remote_sharder, remote_shard, 0, strategy);
            };
            auto reader1 = create_reader(strategy1);
            auto reader2 = create_reader(strategy2);
            std::optional<sstring> mismatch;
            while (true) {
                const auto mf1 = co_await reader1.read_mutation_fragment();
                const auto mf2 = co_await reader2.read_mutation_fragment();
                if (mf1 && mf2) {
                    if (mf1->equal(s, *mf2)) {
                        continue;
                    }
                    ...
                    mismatch = mf1 ?
                        ::format("{} != <>", mutation_fragment::printer(s, *mf1)) :
                        ::format("<> != {}", mutation_fragment::printer(s, *mf2));
                }
                break;
            }

            co_await reader1.on_end_of_stream();
            co_await reader1.close();
            co_await reader2.on_end_of_stream();
